...
本目录**只放构建脚本**,不放构建产物。官方插件由 CI 构建后发布到孤儿分支 `plugins-dist`,清单包含 schema、节点命名空间、权限、CSP、依赖完整性与 SHA-256 内容寻址 bundle URL。画布生产环境只接受 allowlisted 官方注册表；任意 JS URL 安装不属于生产流程。
