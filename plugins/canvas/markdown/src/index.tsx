// Markdown 节点:编辑与渲染 Markdown。Parser is bundled so no runtime dependency can drift.
...
function renderMarkdown(source: string): string {
  ...
  if (out === undefined) {
    out = marked.parse(key) as string;
...
function MarkdownPreview({ ctx }: CanvasNodeContentProps) {
  ...
  const html = ctx.sanitizeMarkup("markdown", renderMarkdown(source));
  ...
  return (
    <div
      ...
      style={{ height: "100%", width: "100%", color: ctx.theme.node.text }}
    />
  );
}
...
function MarkdownContent({ ctx }: CanvasNodeContentProps) {
  return ctx.node.metadata?.editing ? (
    <MarkdownEditor ctx={ctx} />
  ) : (
    <MarkdownPreview ctx={ctx} />
  );
}
