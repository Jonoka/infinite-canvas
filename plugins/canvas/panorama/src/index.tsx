...
// three.js is bundled at a pinned version. AI generation uses the host panel.
import {
  definePlugin,
  useCallback,
  useEffect,
    ...
    loadThree()
      .then((THREE: any) => {
        ...
        const onMove = (event: PointerEvent) => {
          ...
          lat = Math.max(
            -84,
            Math.min(84, startLat + (event.clientY - startY) * 0.12),
          );
        };
        const onUp = () => {
          dragging = false;
        ...
        <div
          style={{
            ...
            background:
              status === "error"
                ? "rgba(0,0,0,0.6)"
                : `linear-gradient(135deg, ${ctx.theme.node.fill}, #000)`,
          }}
        >
          {status === "error"
            ? "全景图读取失败,请换 2:1 的 JPG/PNG 全景图"
            : "正在加载全景…"}
        </div>
      ) : null}
    </div>
...
function PanoramaEmpty({ ctx }: { ctx: CanvasNodeContext }) {
  ...
  const onPick = useCallback(
    (file: File | undefined) => {
      ...
      reader.onload = () =>
        ctx.updateMetadata({ content: String(reader.result || "") });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          ...
          style={{
            ...baseBtn,
            border: "none",
            background: t.toolbar.activeBg,
            color: t.toolbar.activeText,
          }}
      ...
      <span
        style={{ fontSize: 12, color: t.node.placeholder, textAlign: "center" }}
      >
        支持 2:1 等距柱状全景图
      </span>
    </div>
...
function PanoramaContent({ ctx }: CanvasNodeContentProps) {
  ...
  const source =
    (typeof ctx.node.metadata?.content === "string"
...
export default definePlugin({
  ...
  version: "1.1.0",
  description:
    "查看 360° 等距柱状全景图,支持上传与 AI 生成,可从上游图片节点取图",
  nodes: [
    {
      ...
      useBuiltinPanel: {
        mode: "image",
        promptPrefix: PANORAMA_SYSTEM_PROMPT,
        writeBackToSelf: true,
      },
      ...
      toolbar: (ctx) => {
        const hasOwnContent =
          typeof ctx.node.metadata?.content === "string" &&
          Boolean(ctx.node.metadata?.content);
        return [
          {
