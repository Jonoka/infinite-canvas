...
function SvgContent({ ctx }: CanvasNodeContentProps) {
  ...
  const upstream = ctx
    ...
    .find(
      (text): text is string =>
        typeof text === "string" && text.trim().startsWith("<svg"),
    );
  const value = stored ?? "";
  const svg = ctx.sanitizeMarkup("svg", value.trim() || upstream || "");

  // 无自身内容但有上游 SVG:自动采用上游源码
  useEffect(() => {
    if (stored === undefined && upstream)
      ctx.updateMetadata({ content: upstream });
  }, [ctx, stored, upstream]);
  ...
  const toggle = {
    ...
    background: `${ctx.theme.toolbar.panel}dd`,
