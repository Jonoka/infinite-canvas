...
export function createCanvasImageActions<T>(dependencies: CanvasImageDependencies<T>) {
    ...
    return {
        run: (surface: string, config: AiConfig, prompts: string[], results?: PromiseSettledResult<T>[]) => actions.generateBatch(config, prompts.map((prompt) => prompt || surface), results),
    };
}
