...
import { assertModelCapability, resolveModelRequestConfig, resolveModelScript, type AiConfig } from "@/stores/use-config-store";
...
export async function requestAudioGeneration(config: AiConfig, prompt: string, options?: RequestOptions): Promise<Blob> {
    const selectedModel = config.audioModel?.trim() || config.model.trim();
    assertModelCapability(config, selectedModel, "audio", "音频");
    ...
    try {
        const response = await axios.post<Blob>(
            ...
            aiRequestOptions(requestConfig, { responseType: "blob", signal: options?.signal }),
        );
...
function assertAudioConfig(config: AiConfig, model: string) {
    assertAiConfig(config, model, "音频");
