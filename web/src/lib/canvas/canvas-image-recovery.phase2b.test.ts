...
type RecoveryHooks = {
    ...
    clearImageTaskRecovery: <T extends RecoveryMetadata>(metadata: T) => T;
    ...
    executeImageTaskRecovery: (input: {
        node: CanvasNodeData;
        request: (request: { method: "GET" | "POST"; path: string }) => unknown | Promise<unknown>;
    }) => Promise<Blob>;
};
...
describe("Phase 2B billing-safe image recovery eligibility", () => {
    ...
    test("recovery executor performs only planned GETs and returns the fetched image content", async () => {
        const image = new Blob(["recovered-image"], { type: "image/png" });
        const requests: Array<{ method: "GET" | "POST"; path: string }> = [];

        const result = await hook("executeImageTaskRecovery")({
            node: node({}, { taskId: "task/content", taskContentIndex: 4 }),
            request: async (request) => {
                requests.push(request);
                if (request.method === "POST") throw new Error(`billable POST attempted: ${request.path}`);
                if (request.path === "/images/tasks/task%2Fcontent") return { code: 0, data: { status: "completed" } };
                if (request.path === "/images/tasks/task%2Fcontent/content/4") return image;
                throw new Error(`unplanned request: ${request.path}`);
            },
        });

        expect(result).toBe(image);
        expect(requests).toEqual([
            { method: "GET", path: "/images/tasks/task%2Fcontent" },
            { method: "GET", path: "/images/tasks/task%2Fcontent/content/4" },
        ]);
    });
});
...
describe("Phase 2B interruption and new-submission semantics", () => {
    ...
    test("beginning a real new submission clears stale recovery fields and preserves unrelated metadata", () => {
        ...
        expect((metadata as { sentinel?: unknown }).sentinel).toBe(sentinel);
    });
});
