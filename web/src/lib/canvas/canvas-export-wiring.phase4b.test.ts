...
const canvasIndex = await Bun.file(new URL("../../pages/canvas/index.tsx", import.meta.url)).text();
const projectCard = await Bun.file(new URL("../../components/canvas/canvas-project-card.tsx", import.meta.url)).text();
...
describe("Phase 4B export wiring", () => {
    ...
    test("all export entry points consume results and catch failures", () => {
        for (const source of [canvasIndex, projectCard]) {
            expect(source).toContain("const result = await exportCanvasProjects");
            expect(source).toContain('result.status === "partial"');
            expect(source).toContain("catch (error)");
        }
        expect(canvasIndex).toContain("parseCanvasProjectExportManifest");
        expect(sidePanel).toContain("catch (error)");
        expect(projectPage).toContain("catch (error)");
    });
});
